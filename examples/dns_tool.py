...
def add_record(client, domain, record_data, args=None):
    ...
    try:
        ...
        if not getattr(args, 'force', False) and not confirm_action("Do you want to proceed?"):
        ...
        if result.get("DomainDNSSetHostsResult", {}).get("IsSuccess"):
            print(f"Successfully added {new_record['RecordType']} record for {new_record['HostName']}.{domain}")
        else:
    ...
    try:
        ...
        for record in host_records:
            ...
            if (record.get("Name") == name and 
                ...
                (not value or record.get("Address") == value)):
        ...
        if not getattr(args, 'force', False) and not confirm_action(f"Do you want to delete {len(records_to_delete)} record(s)?"):
            print("Operation cancelled.")
            return
        ...
        if result.get("DomainDNSSetHostsResult", {}).get("IsSuccess"):
            print(f"Successfully deleted {len(records_to_delete)} {record_type} record(s) for {name}.{domain}")
        else:
...
def import_records(client, domain, json_file, args=None):
    ...
    try:
        ...
        if not getattr(args, 'force', False) and not confirm_action("Do you want to proceed with the import?"):
...
def main():
    ...
    add_parser.add_argument("--type", choices=["A", "AAAA", "CNAME", "MX", "TXT", "URL", "URL301", "FRAME"], required=True, help="Record type")
    add_parser.add_argument("--value", required=True, help="Record value/address")
    add_parser.add_argument("--ttl", default=1800, type=int, help="Time to live in seconds")
    add_parser.add_argument("--priority", default=10, type=int, help="Priority (for MX records)")
    add_parser.add_argument("--force", action="store_true", help="Skip confirmation prompt")
    
    # Delete record command
    delete_parser = subparsers.add_parser("delete", help="Delete a DNS record from a domain")
    delete_parser.add_argument("domain", help="Domain name")
    delete_parser.add_argument("--name", required=True, help="Host name to delete")
    delete_parser.add_argument("--type", required=True, choices=["A", "AAAA", "CNAME", "MX", "TXT", "URL", "URL301", "FRAME"], help="Record type to delete")
    delete_parser.add_argument("--value", help="Record value/address (if specified, only deletes records with this value)")
    delete_parser.add_argument("--force", action="store_true", help="Skip confirmation prompt")
    
    # Import records command
    import_parser = subparsers.add_parser("import", help="Import DNS records from a JSON file")
    ...
    export_parser = subparsers.add_parser("export", help="Export DNS records to a JSON file")
