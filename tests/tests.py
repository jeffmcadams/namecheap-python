...
class TestNamecheapClient(unittest.TestCase):
    ...
    def test_make_request(self, mock_get):
        ...
        if isinstance(domain_check_result, list):
            ...
            self.assertEqual(domain_check_result[0]["Available"], False)  # Should be converted to a bool
        else:
            # If it's a single item
            self.assertEqual(domain_check_result["Domain"], "example.com")
            self.assertEqual(domain_check_result["Available"], False)  # Should be converted to a bool
    ...
    def test_error_response(self, mock_get):
        ...
        self.assertEqual(context.exception.message, "API Key is invalid or API access has not been enabled - Please verify your API key and ensure API access is enabled at https://ap.www.namecheap.com/settings/tools/apiaccess/")
    ...
    def test_domains_get_list(self, mock_make_request):
        ...
        mock_make_request.return_value = {
            "DomainGetListResult": {
                "Domain": [
                    {"ID": "1", "Name": "example.com", "Created": "2023-01-01", "Expires": "2024-01-01"},
                    {"ID": "2", "Name": "example.net", "Created": "2023-01-01", "Expires": "2024-01-01"}
                ]
            },
        ...
        self.assertEqual(result["DomainGetListResult"]["Domain"][0]["Name"], "example.com")
        self.assertEqual(result["DomainGetListResult"]["Domain"][1]["Name"], "example.net")
    ...
    def test_domains_dns_set_hosts(self, mock_make_request):
        ...
        hosts = [
            ...
            {"HostName": "mail", "RecordType": "MX", "Address": "mail.example.com", "MXPref": "10", "TTL": "1800"}
        ]
        ...
        with self.assertRaises(ValueError):
            self.client.domains_dns_set_hosts("example.com", [{"RecordType": "A", "Address": "192.0.2.1"}])
            
        with self.assertRaises(ValueError):
            self.client.domains_dns_set_hosts("example.com", [{"HostName": "@", "Address": "192.0.2.1"}])
            
        with self.assertRaises(ValueError):
            self.client.domains_dns_set_hosts("example.com", [{"HostName": "@", "RecordType": "A"}])
            
        # Test with invalid record type
        with self.assertRaises(ValueError):
            self.client.domains_dns_set_hosts("example.com", [
        ...
        mx_host = {"HostName": "mail", "RecordType": "MX", "Address": "mail.example.com"}
        self.client.domains_dns_set_hosts("example.com", [mx_host])
            
        # Test with invalid TTL
        with self.assertRaises(ValueError):
            self.client.domains_dns_set_hosts("example.com", [
                {"HostName": "@", "RecordType": "A", "Address": "192.0.2.1", "TTL": "30"}
            ])
